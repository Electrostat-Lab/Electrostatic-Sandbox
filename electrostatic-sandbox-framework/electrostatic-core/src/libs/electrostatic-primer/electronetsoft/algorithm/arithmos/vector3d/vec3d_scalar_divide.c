...
status_code vec3d_scalar_divide(vector3d v, vec_component scalar,
    ...
    __code = vec2d_scalar_divide(v_xy, scalar, &xy_out, NULL);
    if (PASS != __code) {
        return __code;
    }

    __code = vec2d_scalar_divide(v_z, scalar, &z_out, NULL);
    if (PASS != __code) {
        return __code;
    }
