...
- **Formula**: Recall that _A_ is a set of objects of cardinality $$|A| = n$$ members;
   ...
   * A permutation of A taken _r_ at a time is the number of all the possible subsequences of A taken _r_ at a time; such that $$1 <= r <= n$$, and **given the property of allowed repetitions of the permutated objects**; such that the first member $m_0$ repeats $k_0$ number of times, the second member $m_1$ repeats $k_1$ number of times, and so on till member $m_{n-1}$; where n is the total number of objects in set A: $$_rP_n^{\\{k_0, k_1, ...k\_{n-1}\\}} = \frac{\prod\_{i = 0}^{(r - 1)} (n - i)}{\prod\_{m = 0}^{(n - 1)} (\prod\_{j = 0}^{(k - 1)} (k - j))_m} = \frac{n!}{k_0.k_1...k_n.(n - r)!}$$.
