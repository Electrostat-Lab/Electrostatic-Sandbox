...
> 5) _Non-deterministic Automaton_: a non-deterministic machine defines a non-unique transition function for each pair of state $q_n$ and input $\sigma_n$, in other words the transition from the state $q_n$ with the input $\sigma_n$ is not pre-determined 
